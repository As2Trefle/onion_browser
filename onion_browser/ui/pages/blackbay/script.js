(function () {
  // Mesure omnibox (si exposée par le router)
  ...
  const mo = new MutationObserver(() => {
    if (root && !document.body.contains(root)) {
      // cleanup animation + listeners à l’unmount
      stopAnim();
  ...
  if (tabs) {
    ...
    // aucun onglet actif par défaut sur la home, mais on garde la nav fonctionnelle
  ...
  const marginTop = 32, marginBottom = 32, marginLeft = 32, marginRight = 32;
  ...
  function resizeCanvas() {
    if (!canvas) return;
    // taille visuelle (pas de devicePixelRatio pour rester léger dans lb-phone)
  ...
  function onResize() {
    resizeCanvas();
    // recalcule le padding bas au besoin
