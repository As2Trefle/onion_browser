// Mesure de la hauteur réelle de l’omnibox → variable CSS --omnibox-h
...
(function () {
  ...
  function render() {
    ...
    if (!cart.length) {
      listEl.innerHTML = `<div class="bbEmpty">Votre panier est vide.</div>`;
      // badge éventuel dans la sous-nav des autres pages
      ...
      <article class="bbCartItem" data-id="${it.id}">
        ...
        <div class="bbCartItem__main">
          ...
          <div class="bbCartItem__foot">
            <div class="bbCartItem__unit">${fmtUSD(it.price)}</div>
  ...
  // Délégation des clics (qty / remove / clear / continue)
  listEl.addEventListener('click', (e) => {
    ...
    // Actions globales du bloc récap
    ...
    if (e.target.closest('[data-act="continue"]')) {
      if (window.OnionRouter) window.OnionRouter.go('blackbay_drogues');
      e.preventDefault();
    }
  });
  ...
  if (tabs) {
    tabs.addEventListener('click', (e) => {
      ...
      if ((t === 'armes' || t === 'marche') && window.OnionRouter) return window.OnionRouter.go('blackbay');
    });
  }
