// Mesure de la hauteur réelle de l’omnibox (comme sur Drogues)
...
(function () {
  ...
  function render() {
    ...
    if (!cart.length) {
      ...
      // Recalcule immédiatement l’offset bas
    ...
    // Recalcule l’offset bas après insertion (images, etc.)
    requestAnimationFrame(() => window.dispatchEvent(new Event('resize')));
  }
  ...
  const tabs = $('#bb-tabs');
  if (tabs) {
    // ⚠️ Aucun onglet actif sur la page Panier
    ...
    tabs.addEventListener('click', (e) => {
      ...
      if (t === 'marche') return window.OnionRouter.go('blackbay');
    });
  }
