...
(function () {
  ...
  const Router = {
    ...
    async go(pageName = 'home') {
      ...
      // Idempotence : recliquer "Panier" depuis "Panier" ne fait rien
      ...
      try {
        ...
        // sécurité & mesure une fois la page prête
        ...
        pageScript.addEventListener('error', () => {
          ...
          // même en erreur, on évite tout décalage d’UI
          setTimeout(() => window.OnionRouter?.measureOmnibox?.(), 0);
        });
  ...
  // Expose global (merge pour conserver measureOmnibox & co)
