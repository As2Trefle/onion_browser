...
local WEBHOOK_DEBUG  = true                                -- => trace détaillée en console
...
-- ===========================
--  DB wrappers (oxmysql MySQL.* / ghmattimysql)
-- ===========================
local HAS_GHMA  = GetResourceState('ghmattimysql') == 'started'
local HAS_MYSQL = type(MySQL) == 'table'   -- @oxmysql/lib/MySQL.lua

local function db_query(sql, params, cb)
    cb = cb or function(_) end
    if HAS_MYSQL then
        MySQL.query(sql, params or {}, cb)             -- SELECT
...
local function db_insert(sql, params, cb)
    ...
    if HAS_MYSQL and MySQL.insert then
        MySQL.insert(sql, params or {}, cb)            -- retourne insertId
    elseif HAS_GHMA then
...
local function sendDiscord(payload)
    ...
    -- Pas de username/avatar: on laisse ceux du webhook
...
QBCore.Functions.CreateCallback('onion:bb_update_annonce', function(source, cb, data)
    ...
    -- Récupérer l’ancien en FR pour le log
    db_query([[
...
-- ===========================
--  COMMANDE TEST WEBHOOK
-- ===========================
RegisterCommand('bb_ann_logtest', function(src, args)
    if src ~= 0 then return end -- serveur/console only
    local fakePlayer = {
        PlayerData = {
